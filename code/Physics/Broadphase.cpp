...
void SortBodiesBounds(const Body* bodies, const int num, psuedoBody_t* sortedArray, const float dt_sec) {
	...
	for (int i = 0; i < num; i++) {
		...
		bounds.Expand(bounds.mins + Vec3(-1) * dt_sec);
		bounds.Expand(bounds.maxs + Vec3(1) * dt_sec);

		sortedArray[i * 2].id = i;
		sortedArray[i * 2].value = axis.Dot(bounds.mins);
...
void SweepAndPrune1D(const Body* bodies, const int num, std::vector<collisionPair_t>& finalPairs, const float dt_sec) {
	psuedoBody_t* sortedBoodies = (psuedoBody_t*)alloca(sizeof(psuedoBody_t) * num * 2);

	SortBodiesBounds(bodies, num, sortedBoodies, dt_sec);
	BuildPairs(finalPairs, sortedBoodies, num);
}
